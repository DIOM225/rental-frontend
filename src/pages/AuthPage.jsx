...
function AuthPage() {
  ...
  const [form, setForm] = useState({
    ...
    phone: '',
    birthday: '',
    identifier: '', // login only (phone OR email)
    password: '',
    confirmPassword: '',
  });
  ...
  const handleChange = (e) => {
    ...
    if (name === 'phone') {
      cleanValue = value.replace(/\D/g, ''); // keep only numbers
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        payload = {
          identifier: form.identifier.trim(), // phone OR email
      ...
      // ✅ Always log in immediately (login + signup)
      saveAndRedirect(res);
...
const styles = {
  container: {
    ...
    height: '90vh', backgroundColor: '#f7f7f7',
  },
  card: {
    background: '#fff', padding: '2rem', borderRadius: '12px',
    boxShadow: '0 8px 20px rgba(0,0,0,0.08)', width: '100%', maxWidth: '400px',
  },
  title: {
    fontSize: '1.8rem', textAlign: 'center', marginBottom: '1.5rem',
  },
  form: {
  ...
  button: {
    backgroundColor: '#007bff', color: '#fff', padding: '0.75rem',
    borderRadius: '8px', border: 'none', cursor: 'pointer',
    fontWeight: 'bold', fontSize: '1rem',
  },
  links: {
    marginTop: '1.2rem', textAlign: 'center', fontSize: '0.9rem',
  },
  link: {
